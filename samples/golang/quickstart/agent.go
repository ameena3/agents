...
// RegisterHandlers sets up the message routing for the quickstart agent
func RegisterHandlers(app *AgentApp) {
	app.OnConversationUpdate("membersAdded", func(ctx *TurnContext) {
		if err := ctx.SendActivity("Welcome to the empty agent! This agent is designed to be a starting point for your own agent development."); err != nil {
			log.Printf("Error sending welcome: %v", err)
		}
	})

	app.OnMessage(regexp.MustCompile(`(?i)^hello$`), func(ctx *TurnContext) {
